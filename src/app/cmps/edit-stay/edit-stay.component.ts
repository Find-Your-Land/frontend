...
export class EditStayComponent {
  ...
  async onAddStay () {
    ...
    try {
      ...
      this.snackBar.open('Local Salvo com sucesso', 'Fechar', { duration: 3000 })
  ...
  checkValidation () {
    ...
    if (stay.imgUrls.length < 5) {
      this.snackBar.open('Voce deve add 5 imagens', 'Fechar', { duration: 3000 })
      return false
    }
    if (stay.capacity < 1) {
      this.snackBar.open('Voce deve addicionar pelo menos 1', 'Fechar', { duration: 3000 })
      return false
    }
    if (stay.name === '') {
      this.snackBar.open('Voce deve addicionar nome do local', 'Fechar', { duration: 3000 })
      return false
    }
    if (stay.price < 1) {
      this.snackBar.open('Voce deve addicionar preco valido', 'Fechar', { duration: 3000 })
      return false
    }
    if (!/^[a-zA-Z]{3,}$/.test(stay.loc.country)) {
      this.snackBar.open('Voce deve addicionar pais valido', 'Fechar', { duration: 3000 })
      return false
    }
    if (!/^[a-zA-Z]{2,}$/.test(stay.loc.city)) {
      this.snackBar.open('Voce deve addicionar cidade valida', 'Fechar', { duration: 3000 })
      return false
    }
    if (!/^[a-zA-Z][a-zA-Z0-9\s]{2,}$/.test(stay.loc.address)) {
      this.snackBar.open('Voce deve addicionar endereco valido', 'Fechar', { duration: 3000 })
      return false
    }
    return true
  }
